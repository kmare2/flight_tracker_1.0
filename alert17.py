...
async def main_loop():
    ...
    refresh_ready = False
    refresh_ready_time = None  # <-- New variable to track when refresh_ready became True
    ...
    with Live(render_dashboard(aircraft_list, latest_alert, spinner_frames[spinner_index], within_schedule), refresh_per_second=1, screen=True) as live:
        while True:
            ...
            if not within_schedule:
                ...
                # Also reset refresh flags since display is off
                refresh_ready = False
                ...
                if matching_aircraft:
                    ...
                    if start_refresh_cycle:
                        ...
                        refresh_ready = False
                        refresh_ready_time = None  # Reset refresh ready time when starting a new cycle
                        ...
                        # Launch card5.py once at the start of the refresh cycle
                    ...
                    # Check if PNG updated, set refresh_ready and record time when first detected
                    current_png_mtime = get_mtime(png_path)
                    if not refresh_ready and current_png_mtime > last_png_mtime:
                        refresh_ready = True
                        refresh_ready_time = now  # <-- Start timer here!
                    ...
                    if refresh_ready and refresh_ready_time is not None:
                        ...
                        if elapsed_ms <= REFRESH_DURATION_MS:
                            ...
                            # Refresh window expired — reset flags to allow next cycle
                            refresh_ready = False
                            refresh_ready_time = None
