...
# Add your existing PUBLISH_SCHEDULE and DISABLED_MONTHS here, for example:
PUBLISH_SCHEDULE = {
    'mon': [('16:00', '20:00')],
    'tue': [('16:00', '20:00')],
    'wed': [('16:00', '20:00')],
...
REFRESH_DURATION_MS = 18000        # keep refresh=True for this many ms after cycle start
...
def is_within_schedule(now):
    """Return True if now (converted to EST) is within PUBLISH_SCHEDULE and not in DISABLED_MONTHS."""
    # Convert naive datetime to EST assuming naive means UTC; adjust if needed
    if now.tzinfo is None:
        now_est = now.replace(tzinfo=ZoneInfo("UTC")).astimezone(EST)
    else:
        now_est = now.astimezone(EST)

    month_name = now_est.strftime("%b").lower()
    if month_name in DISABLED_MONTHS:
        return False

    day_name = now_est.strftime("%a").lower()
    schedule = PUBLISH_SCHEDULE.get(day_name)
    if not schedule:
        return False
...
async def main_loop():
    global spinner_index, last_alert_write_time, latest_alert, current_temperature_c, last_refresh_start,last_card_launch_time,within_schedule
    ...
    with Live(render_dashboard(aircraft_list, latest_alert, spinner_frames[spinner_index], within_schedule), refresh_per_second=1, screen=True) as live:
        while True:
            ...
            # Check schedule and month restrictions
            ...
            for ac in data.get("aircraft", []):
                ...
                is_closing = ( bullseye_km is not None and bullseye_km <= BULLSEYE_ALERT_KM and hexcode not in BLACKLISTED_HEX ) 
            ...
            if not within_schedule:
                # Outside schedule or disabled month → turn off display
                latest_alert = {
                    "display": False,
                    "refresh": False,
                ...
                last_alerted_flight = None  # Reset last alerted flight
            else:
                # Filter aircraft with flight number, within bullseye and within 5km
                matching_aircraft = [
                ...
                if matching_aircraft:
                    ...
                    # If starting a refresh, do the heavier work (scrape + card render) ONCE
                    if start_refresh_cycle:
                        last_refresh_start = now
                        refresh_ready = False
                        last_png_mtime = get_mtime(png_path)
                        # fetch FlightAware info once at the start of the refresh cycle
                        try:
                            fa_info = await scrape_flightaware(flight) if flight else {}
                        except Exception:
                            fa_info = {}
                        gc.collect()
                        # store/fa_info so subsequent cycles can reuse it while active
                        aircraft_list[hexcode]["flightaware"] = fa_info

                        # compute temperature (you can keep calling this every cycle if you like)
                        temperature_c = get_temperature(ac["lat"], ac["lon"])
                        current_temperature_c = temperature_c

                        # compute ETA and percent_complete as before
                        eta_minutes = None
                        try:
                        ...
                        # Launch card3.py once at the start of the refresh cycle
                        subprocess.Popen([sys.executable, 'card5.py'],
                                         stdout=subprocess.DEVNULL,
                                         stderr=subprocess.DEVNULL)
                        # record the card launch time (same as last_refresh_start)
                        last_card_launch_time = last_refresh_start

                    else:
                        # Not starting a new refresh cycle:
                        # - reuse previously-scraped flightaware info if present
                        fa_info = aircraft_list.get(hexcode, {}).get("flightaware", {}) or {}
                        # You may optionally refresh temperature every cycle for the header:
                        temperature_c = get_temperature(ac["lat"], ac["lon"])
                        current_temperature_c = temperature_c

                        # compute ETA/percent from fa_info if available
                        eta_minutes = None
                        try:
                            landing_time_str = fa_info.get("landing_time_estimated")
                            if landing_time_str:
                    ...
                    # Decide refresh flag based on time since last_refresh_start
                    current_png_mtime = get_mtime(png_path)
                    if not refresh_ready and current_png_mtime > last_png_mtime:
                        refresh_ready = True
                    
                    refresh_flag = False
                    if refresh_ready and last_refresh_start is not None:
                        elapsed_ms = (now - last_refresh_start).total_seconds() * 1000.0
                        if elapsed_ms <= REFRESH_DURATION_MS:
                            refresh_flag = True

                    # Build latest_alert and write JSON every cycle
                    latest_alert = {
                        "display": True,
                        "refresh": refresh_flag,
                        "refreshInterval": 10000,
                        "png_url": "http://192.168.0.105:8080/flight_card.png",
                        "flight": flight,
                        "aircraft_info": ac["adsb"],
                        "flight_info": fa_info,
                        "eta_minutes": eta_minutes,
                        "speed": ac.get("speed"),
                        "heading": ac.get("heading"),
                        "altitude": ac.get("altitude"),
                        "bullseye_km": ac.get("bullseye_km"),
                        "flight_progress": 100 - (fa_info.get("distance_remaining_nm", 0) / max((fa_info.get("distance_elapsed_nm", 1) + fa_info.get("distance_remaining_nm", 0)), 1)) * 100 if fa_info.get("distance_remaining_nm") and fa_info.get("distance_elapsed_nm") else 0,
                    ...
                    # mark aircraft as alerted (optional - no longer gated by flight identity)
                    aircraft_list[ac["hex"]]["alerted"] = True

                else:
                    if latest_alert is None:
                    # No matching aircraft → turn off display
                        latest_alert = {
                            "display": True,
                            "refresh": False,
                            "refreshInterval": 10000,
